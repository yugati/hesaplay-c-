...
async function linkKapsami(row) {
  ...
  const [specRes, katlar, srMap] = await Promise.all([
...
async function acikGonder(req, res, token) {
  ...
  for (let i = 0; i < gelen.length; i++) {
    ...
    const sat = { specId: kalem.id, kod: kalem.kod, ad: kalem.ad, ozelNo: kalem.ozelNo, birim: kalem.birim, sart: kalem.sart, miktar, katId, katAd, kat: katMetin }
