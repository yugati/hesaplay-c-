...
//   tur='link'     : data {tarih,bina,cat,ekip,vardiya,sart,katId,katAd,stokDus,
//                          sure,bitis,olusturan,iptal,iptalEden,iptalTs}
...
async function sartnameBul(org, tasari, bina, code) {
  const { data, error } = await supabaseAdmin.from('proje_sartnames')
    .select('id, katIds:data->katIds, silindi:data->>is_deleted')
    .eq('org_id', org).eq('tasari_id', tasari)
    .eq('data->>bina', bina).eq('data->>code', code).limit(5)
  if (error) throw error
  return (data || []).find(x => x.silindi !== 'true') || null
}
...
/* Linkin sayfada gosterecegi kalemler ve kat secenekleri. Onizleme ve gonderim
   ...
   Kalem kumesi: bu binada, bu bolumde, (sartname seciliyse) grubu o sartname olan,
   silinmemis sartname kalemleri. Birim ve foto once kutuphaneden (index.html
   rLineLibUnit / specImgOf ile ayni oncelik). Egress icin yalnizca gereken alanlar
   cekilir - kalemin tam govdesi (lokTargets vb.) inmez. */
async function linkKapsami(row) {
  const org = row.org_id, tas = row.tasari_id, d = row.data || {}
  const [specRes, katlar, sr] = await Promise.all([
    ...
    d.sart ? sartnameBul(org, tas, d.bina, d.sart) : Promise.resolve(null),
  ])
  if (specRes.error) throw specRes.error
  let specs = (specRes.data || []).filter(s => s.silindi !== 'true')
  if (d.sart) specs = specs.filter(s => grupAnahtari(s) === d.sart)

  const matIds = [...new Set(specs.map(s => s.matId).filter(Boolean))]
  const mats = {}
  for (let i = 0; i < matIds.length; i += 150) {
    const { data, error } = await supabaseAdmin.from('proje_materials')
      .select('id, unit:data->>unit, img:data->>img').eq('org_id', org).in('id', matIds.slice(i, i + 150))
    if (error) throw error
  ...
  const kalemler = specs.map(s => {
    ...
    return {
      ...
      img: foto(m && m.img, s.img),
    }
  }).sort((a, b) => a.kod.localeCompare(b.kod, 'tr', { numeric: true }) || a.ad.localeCompare(b.ad, 'tr'))

  // Kat secenekleri: sartname katlara atanmissa yalnizca onlar (index.html sartKatIds),
  // degilse binanin tum katlari. Link tek kata sabitlendiyse secim yoktur.
  let katSecim = katlar
  const katIds = sr && Array.isArray(sr.katIds) ? sr.katIds : []
  if (katIds.length) {
    const set = new Set(katIds)
...
async function acikOnizle(req, res, token) {
  ...
  res.status(200).json({
    link: { tarih: d.tarih, bina: d.bina, cat: d.cat, ekip: d.ekip, vardiya: d.vardiya, sart: d.sart || '', bitis: d.bitis },
    katSabit: k.katSabit, katlar: k.katlar, kalemler: k.kalemler,
    gonderimSayisi: sayi, maxGonderim: MAX_GONDERIM,
  })
}
...
async function acikGonder(req, res, token) {
  ...
  for (let i = 0; i < gelen.length; i++) {
    ...
    const sat = { specId: kalem.id, kod: kalem.kod, ad: kalem.ad, ozelNo: kalem.ozelNo, birim: kalem.birim, miktar, katId, katAd, kat: katMetin }
  ...
  const data = {
    tarih: d.tarih, bina: d.bina, cat: d.cat, ekip: d.ekip, vardiya: d.vardiya, sart: d.sart || '',
...
async function linkOlustur(res, claims, org, tasari, g) {
  ...
  const sart = metin(g.sart, 160), katId = metin(g.katId, 80)
  ...
  // yanlis secim bos bir sayfa uretmesin: sartname ve kat bu binada olmali
  if (sart && !(await sartnameBul(org, tasari, bina, sart))) return hata(res, 400, 'Secilen sartname bu binada bulunamadi')
  let katAd = ''
  if (katId) {
  ...
  const id = yeniId()
  const data = {
    tarih, bina, cat, ekip, vardiya, sart, katId, katAd, stokDus: g.stokDus !== false,
  ...
  await denetimYaz(org, {
    ...
    detail: `Saha linki olusturuldu: ${tarih} ${bina} ${vardiya} - ${cat} / ${ekip}${sart ? ' / ' + sart : ''}${katAd ? ' / kat ' + katAd : ''}`,
  })
