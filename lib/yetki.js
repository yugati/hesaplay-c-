...
const YAZMA_MODULU = {
  ...
  /* KATALOG: Ihtiyac Listesi ekraninin icindeki alt bolum ama o sistemden BAGIMSIZ -
     Malzeme Kutuphanesi'nden secilen bir urune ozellik (marka/model/olcu/malzeme)
     eklenir. Yetkisi bu yuzden 'siparis' degil, kutuphaneyle AYNI moduldendir. */
  katalog: 'kutuphane',
...
const OKUMA_MODULLERI = {
  ...
  // Katalog Ihtiyac Listesi sayfasinin ICINDE gorunur, okuma yetkisi bu yuzden
  // ayni: sayfayi acabilen (siparis/proje) katalog sekmesini de gorur. Yazma
  // yine 'kutuphane'ye bagli (bkz. YAZMA_MODULU) - gorme ile duzenleme ayri.
  katalog: ['siparis', 'proje'],
...
// audit_log: yazma herkese acik (her kullanici kendi eylemini kaydeder) - ama kimlik
// alanlari artik sunucuda damgalanir, yani baskasinin adina kayit girilemez
// (bkz. api/veri.js damgala). OKUMA icin bkz. DENETIM_OKUMA.
const SERBEST_YAZMA = new Set(['app_settings', 'audit_log'])
const SERBEST_OKUMA = new Set(['app_settings'])
// Denetim kaydi: rol 'admin' olmak YETMEZ, yetkinin ayrica atanmis olmasi gerekir.
const DENETIM_OKUMA = new Set(['audit_log'])

/* DENETIM KAYDINI KIM GOREBILIR?
   Kural: rol 'admin' (ekranda "Yonetici") OLMAK SART, ama tek basina yetmez -
   yetkinin ayrica ATANMIS olmasi gerekir. Bir yonetici bir baska yoneticiye bu
   yetkiyi verir/alir (bkz. index.html canAudit, api/users.js).

   GERIYE UYUM: alan hic yoksa ACIK sayilir. Bu ozellik gelmeden once denetim
   kaydini butun yoneticiler goruyordu; alan yoklugunu "kapali" saymak, mevcut
   yoneticileri hicbir sey yapmadan ekrandan dusururdu. Yeni/duzenlenen her
   kullaniciya deger ACIKCA yazilir, yani "alan yok" yalnizca eski kayitlarda
   kalir ve ilk kaydetmede kalici hale gelir.

   Yetki users.permissions icinde ('denetim') tutulur: bu alan zaten oturum
   tokenine 'perms' olarak yaziliyor (lib/auth.js), yani her istekte fazladan
   veritabani sorgusu gerekmez. */
export function denetimGorebilir(user) {
  if (!user || user.role !== 'admin') return false
  const p = user.permissions && user.permissions.denetim
  if (!p) return true            // eski kayit: alan yok = acik
  return !!p.read
}

// Oturum tokenindeki iddialardan denetimGorebilir'in bekledigi sekli uretir.
export function tokenKullanici(claims) {
  return { role: claims.role, permissions: claims.perms || {} }
}
...
export function yetkiKontrol(user, table, op, tumTablo) {
  ...
  /* DENETIM KAYDI admin gecisinin ONUNDE durur. Asagidaki "admin ise her sey
     serbest" satirindan sonra konsaydi, yetkisi alinmis bir yonetici ekranda
     dugmeyi gormezdi ama /api/veri'ye elle istek atip kaydi yine okurdu -
     gizlemek kisitlamak degildir. */
  if (DENETIM_OKUMA.has(table)) {
    if (op === 'count' || op === 'select') {
      return denetimGorebilir(user)
        ? { ok: true }
        : { ok: false, kod: 403, mesaj: 'Denetim kaydini goruntuleme yetkiniz yok' }
    }
    // Denetim kaydini SILMEK de ayni yetkiye baglidir (temizleme / toplu silme).
    if (op === 'delete' && !denetimGorebilir(user)) {
      return { ok: false, kod: 403, mesaj: 'Denetim kaydi uzerinde yetkiniz yok' }
    }
  }
...
/* ─── FATURA SAHIPLIGI (satir duzeyi) ─────────────────────────────────────────
   Modul izni (yetkiKontrol) "fatura yazabilir mi" sorusunu cevaplar; bu fonksiyon
   "BU faturaya yazabilir mi" sorusunu. Yonetici disindaki kullanici:
     - YALNIZCA KENDI ACTIGI faturayi duzenler/siler. Yoneticinin ya da baska bir
       kullanicinin actigi fatura, olusturani bilinmeyen eski kayit ona kapalidir.
     - KILITLI faturaya dokunamaz (kilidi yalnizca yonetici acar).
     - Faturayi KILITLEYEMEZ: kilit talep eder (kilitTalep), yonetici onaylar.
     - Onay BEKLEYEN faturayi degistiremez/silemez - once talebi geri ceker.
       Aksi halde yonetici, talep edildigi andan farkli bir belgeyi kilitlerdi.
   Tarayicidaki esi index.html fatSahibiMi / fatDuzenlenebilir / fatSilinebilir;
   ikisi birlikte degisir. eski/yeni: faturanin data (JSONB) govdesi.
   Sonuc: {ok:true} veya {ok:false, kod, mesaj} */
export function faturaYazmaKontrol(user, kullaniciAdi, op, eski, yeni) {
  if (user && user.role === 'admin') return { ok: true }
  const red = mesaj => ({ ok: false, kod: 403, mesaj })
  const kilitli = d => !!d && d.durum === 'kilitli'
  const talepVar = d => !!(d && d.kilitTalep && d.kilitTalep.ts)
  if (op === 'insert') {
    if (kilitli(yeni)) return red('Fatura kilitli olarak kaydedilemez - kilidi yonetici koyar')
    if (talepVar(yeni)) return red('Yeni fatura kilit talebiyle kaydedilemez - once kesinlestirin')
    return { ok: true }
  }
  if (!eski) return red('Fatura bulunamadi - silinmis olabilir, sayfayi yenileyin')
  const ben = String(kullaniciAdi || '')
  if (!ben || eski.olusturan !== ben) {
    return red('Bu faturayi siz acmadiniz - yalnizca kendi actiginiz faturada degisiklik yapabilirsiniz')
  }
  if (kilitli(eski)) return red('Fatura kilitli - kilidi yalnizca yonetici acabilir')
  if (op === 'delete') {
    return talepVar(eski)
      ? red('Kilit onayi bekleyen fatura silinemez - once talebi geri cekin')
      : { ok: true }
  }
  if (op !== 'update') return red('Bu islem faturada yalnizca yoneticiye acik')
  if (kilitli(yeni)) return red('Faturayi kilitleme yetkiniz yok - kilit yonetici onayiyla konur')
  if (talepVar(yeni) && yeni.durum === 'taslak') return red('Taslak fatura icin kilit talep edilemez - once kesinlestirin')
  // talep -> talep: onay beklerken degisiklik. talep -> yok: geri cekme (serbest).
  if (talepVar(eski) && talepVar(yeni)) {
    return red('Fatura kilit onayi bekliyor - degistirmek icin once talebi geri cekin')
  }
  return { ok: true }
}

/* Yonetici disindaki kullanicinin yazdigi fatura govdesinde KIMLIK ve IZ alanlari
   sunucuda sabitlenir - istemcinin gonderdigi deger ezilir:
     - olusturan: eklemede giris yapan kisi, guncellemede kaydin ilk sahibi. Aksi
       halde faturasini yoneticinin adina yazan biri onu herkese kapatabilir ya da
       baskasinin adina belge acabilirdi.
     - sonKilitAcma / kilitSonrasiDegisim: "kilit acildiktan sonra degisti" izi.
       Bu izi yalnizca yonetici koyar/kaldirir; kaydin sahibi silemez.
   Yonetici icin govde oldugu gibi gecer (yedekten geri yukleme tarihi korur). */
export function faturaGovdeSabitle(user, kullaniciAdi, op, eski, yeni) {
  if (!yeni || typeof yeni !== 'object' || (user && user.role === 'admin')) return yeni
  if (op === 'insert') return { ...yeni, olusturan: String(kullaniciAdi || '') }
  if (!eski) return yeni
  return {
    ...yeni,
    olusturan: eski.olusturan,
    olusturmaTs: eski.olusturmaTs != null ? eski.olusturmaTs : yeni.olusturmaTs,
    sonKilitAcma: eski.sonKilitAcma || null,
    kilitSonrasiDegisim: !!(eski.kilitSonrasiDegisim || yeni.kilitSonrasiDegisim),
  }
}
